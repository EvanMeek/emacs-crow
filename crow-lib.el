...
(defun crow--gen-translated-text (content)
  ...
  (set-process-sentinel
   ...
   (lambda (p e)
     (run-at-time crow-translate-delay nil (lambda ()
                                             (let* ((json (ignore-errors (json-parse-string (progn (set-buffer crow-data-buffer-name)
                                                                                                   (buffer-string))
                                                                                            :object-type 'alist))))
                                               (progn (set-buffer (get-buffer-create crow-data-buffer-name))
                                                      (erase-buffer))
                                               (setq crow--translated-text (crow--extract-json-to-translate-text json))
                                               (cond ((equal (car crow-ui-type) 'eldoc) (crow--show-translated-text-by-eldoc))
                                                     ((equal (car crow-ui-type) 'posframe) (crow--show-translated-text-by-posframe)))))))))
...
(defun crow--parser (content-obj)
  ...
  (let (content-text
    ...
    (when (plist-get crow-enable-info :source)
      ...
      (setq content-text (concat content-text source-text (if (plist-get crow-enable-info :translit) "\t" "\n"))))
    (when (plist-get crow-enable-info :translit)
    ...
    (when (plist-get crow-enable-info :translation)
      ...
      (setq content-text (concat content-text translation-text "\n")))
...
(defun crow--parse-translit (obj) "解析并格式化content中的translit元数据" (s-concat "[" obj "]"))
...
(defun crow--check-crow-bin-path()
  (executable-find "crow"))
